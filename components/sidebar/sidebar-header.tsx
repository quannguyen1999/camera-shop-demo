...
import { SkeletonLoading } from "../skeletron-loading";
import { useRouter } from "next/navigation";

interface NavigationSidebarProps {
  isScrolled: boolean;
}
const SideBarHeader = ({ isScrolled }: NavigationSidebarProps) => {
  ...
  return (
    <div
      className={cn(
        "hidden px-4 gap-1 h-16 w-full text-white md:grid sm:grid-cols-1 md:grid-cols-1 lg:grid-col-1 xl:grid-cols-2",
        isScrolled ? "bg-gray-50 shadow-2xl text-black hover:text-black" : ""
      )}
    >
      <div className="flex flex-row gap-5 text-sm md:justify-center lg:justify-center pt-5  xl:justify-start ">
        ...
        {!loading && (
          <div
            ...
            className="cursor-pointer flex flex-row text-center justify-center items-center"
          >
            Liên hệ
          </div>
        )}
